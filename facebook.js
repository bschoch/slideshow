...
function upgradeToken(options) {
  var deferred = q.defer()
  FB.api('/oauth/access_token?grant_type=fb_exchange_token&client_id=' + process.env.VISAUDIO_FACEBOOK_APP_ID + '&client_secret=' + process.env.VISAUDIO_FACEBOOK_APP_SECRET + '&fb_exchange_token=' + options.token, {}, function (res) {
    if (!res || res.error) {
